...
source("https://raw.githubusercontent.com/pujen-shrestha/VisualisationProjects/master/Helper%20Functions.R")
```
...
generate_data <- function(rep, rep_n, mean, sd) {
  data_out <- tidyverse::tibble(group = rep(rep, rep_n),
...
confi_inter <- function(group_id) {
  dataplot <- data_generated %>%
    filter(tidyverse::group == group_id) %>%
    tidyverse::pull(outcome) %>%
    ci_mean()
